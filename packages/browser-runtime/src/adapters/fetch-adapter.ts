...
// HTTP Client Adapter using Axios (Fetch API wrapper)
...
/**
 * Configuration options for the browser HTTP adapter.
 * Uses the shared HttpAdapterConfig interface.
 */
export interface BrowserHttpAdapterConfig extends HttpAdapterConfig {
  /**
...
/**
 * HTTP adapter wrapping Axios for browser runtime.
 * Provides a unified HTTP client interface for pack execution in browsers.
 *
 * This implementation uses Axios which internally uses XMLHttpRequest
 * in browsers, providing broad compatibility and features like
 * request/response interceptors, automatic JSON parsing, and
 * request cancellation via AbortController.
 */
export class FetchAdapter implements IHttpAdapter {
  private readonly config: Required<BrowserHttpAdapterConfig>;
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new HttpAdapterError(
        'FetchAdapter not initialized. Call initialize() first.'
      );
    }
  }
  ...
  addResponseInterceptor(
    ...
    errorInterceptor?: (error: AxiosError<unknown>) => Promise<never> | never
  ): number {
...
/**
 * Create and initialize a fetch adapter with the given configuration.
 * @param config - Adapter configuration
 * @returns Initialized fetch adapter
 */
export async function createFetchAdapter(
  config?: BrowserHttpAdapterConfig
): Promise<FetchAdapter> {
  const adapter = new FetchAdapter(config);
