// Example: PodGroup Greeter Service Pack — Node.js variant
...
//
// Usage:
//   1. Register this as a pack:   stark pack register --name podgroup-greeter-pack-node --file examples/podgroup_greeter/bundle_podgroup_greeter_node.js
//   2. Create a service:          stark service create podgroup-greeter-service --pack podgroup-greeter-pack-node --replicas 1
//   3. The podgroup-echo-service must also be running in the same PodGroup.
//
// Node.js only — see bundle_podgroup_greeter_browser.js for the browser variant.
module.exports.default = async function(context) {
  ...
  const membership = plane.joinGroup(groupId, { ttl: 120_000, metadata: { role: 'greeter', runtime: 'node' } });
  ...
  const intervalId = setInterval(async () => {
    ...
    await plane.joinGroup(groupId, { ttl: 300_000, metadata: { role: 'greeter', runtime: 'node' } });
    ...
    if (otherPodIds.length === 0) {
      console.log(`[${serviceId}] #${seq} No other pods in group "${groupId}" yet (${members.length} member(s) total, mem ${memoryMB}MB)`);
      return;
    }

    console.log(`[${serviceId}] #${seq} Found ${otherPodIds.length} other pod(s) in group — querying /echo (uptime ${now - startTime}ms, mem ${memoryMB}MB)`);
