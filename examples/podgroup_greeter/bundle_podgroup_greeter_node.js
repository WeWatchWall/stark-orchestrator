...
module.exports.default = async function(context) {
  ...
  const group = await plane.joinGroup(groupId, { ttl: 120_000, metadata: { role: 'greeter', runtime: 'node' } });
  ...
  const intervalId = setInterval(async () => {
    ...
    if (otherPodIds.length === 0) {
      console.log(`[${serviceId}] #${seq} No other pods in group "${groupId}" yet (${group.podIds.length} member(s) total, mem ${memoryMB}MB)`);
      return;
    }
