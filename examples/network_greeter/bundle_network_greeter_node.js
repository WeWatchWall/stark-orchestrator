// Example: Greeter Service Pack — Node.js variant
...
//
// Usage:
//   1. Register this as a pack:   stark pack register --name greeter-pack-node --file examples/network_greeter/bundle_network_greeter_node.js
//   2. Create a service:          stark service create greeter-service --pack greeter-pack-node --replicas 1
//   3. The echo-service must be running to receive calls.
//   4. Allow traffic:             stark network allow greeter-service echo-service
//
// Node.js only — see bundle_network_greeter_browser.js for the browser variant.
module.exports.default = async function(context) {
  ...
  const intervalId = setInterval(async () => {
    ...
    const payload = {
      ...
      memoryMB: typeof process !== 'undefined' ? Math.round(process.memoryUsage().heapUsed / 1024 / 1024 * 100) / 100 : null,
    };

    console.log(`[${serviceId}] → Sending greeting #${seq} to echo-service (uptime ${now - startTime}ms, mem ${payload.memoryMB}MB)`);
