...
export class PodConfig { 
    ...
    private async saveInstall() { 
        ...
        let packageDir = path.join( `./packages-run`, this.arg.name);
        ...
        for(let filename of Object.keys(zip.files)) { 
            ...
            let dest = path.join(packageDir, filename);
            fs.outputFileSync(dest, file);
        }
        zip = undefined;

        await execShellCommand(`npm --prefix ./${packageDir} install ./${packageDir}`);
